...
function generateCurvePoints(sol, state, anc, kFactor, kneeM) {
  ...
  // ── Thermal segment: 150 points in log space ─────────────────────────────────
  for (let i = 0; i <= 150; i++) {
    const M = Math.pow(10, logStart + (logEnd - logStart) * i / 150)
...
/**
 * Serializes as a JS object literal element (no quotes on keys).
 * Ready to paste directly inside an array: [..., <paste here>, ...]
 */
function toJSLiteral(key, obj) {
  const fmtPts = (pts) => {
    const lines = []
  ...
  return [
    ...
    `        In: ${obj.In},`,
    `        curves: {`,
    `            mm: ${fmtPts(obj.curves.mm)},`,
    `            tc: ${fmtPts(obj.curves.tc)},`,
    `        },`,
    `    },`,
    `}`,
  ].join('\n')
...
/**
 * Builds the full export string for the current MCB state.
 * Returns a JS object literal (keys unquoted) ready to copy.
 *
...
export function buildExportJSON(state, sol, anc) {
  ...
  return toJSLiteral(key, {
    manufacturer: 'IEC 60898-1',
    type: 'MCB',
    curve_type: state.curveType,
    Icu: 10,
    In: state.In,
    curves: { mm: mmPoints, tc: tcPoints },
  })
}
